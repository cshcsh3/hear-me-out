...
api.interceptors.response.use(
  ...
  (error) => {
    if (error.response) {
      ...
      switch (status) {
        case 409:
          errorMessage = data.detail || 'A conflict occurred with the current state of the resource';
          break;
        case 400:
...
export const apiService = {
  ...
  getTranscription: (transcriptionId) => api.get(`/transcriptions/${transcriptionId}`),
  
  // Search transcriptions
  searchTranscriptions: (query) => api.get('/transcriptions/search', { params: { query } }),
};
